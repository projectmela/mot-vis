...
def frame_from_video(
    video: cv2.VideoCapture, start_frame: int = 0, end_frame: int = -1
):
    ...
    ), f"start_frame ({start_frame}) must be < num_frames ({num_frames})"
    assert (
        end_frame > start_frame
    ), f"end_frame ({end_frame}) must be > start_frame ({start_frame})"
    assert (
        end_frame < num_frames
    ), f"end_frame ({end_frame}) must be < num_frames ({num_frames})"
...
def plot_frames_to_frames(
    ...
    def _process_frame(f_path, i, _mot_dets):
        ...
        plotted_img = plot_mot_bbox_on_image(
            img, _mot_dets[_mot_dets[:, 0] == i]
        )  # BUG FIX : Indexing error earlier used i + 1 used to plot wrong boxes
    ...
    with ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(_process_frame, f_path, i, mot_dets)
            for i, f_path in enumerate(frame_paths)
        ]
        # noinspection PyTypeChecker
        for f in tqdm(
            as_completed(futures), total=len(futures), desc="plotting frames"
        ):
            _ = f.result()
...
def plot_from_video_to_video(
    ...
    mot_anno_path = Path(mot_anno_path).absolute()
    ...
    output_v_path = _get_output_video_path(
        save_path=save_path,
    ...
    for frame_id, img in enumerate(
        tqdm(
            frame_gen,
            total=num_frames,
            desc="plotting frames",
            bar_format="{l_bar}{bar:10}{r_bar}",
        )
    ):
        frame_id += (
            1  # MOT format starts from 1, but frame_id from video capture starts from 0
        )
...
def plot_frames_to_video(
    ...
    if not frame_paths:
        logger.warning("frame_paths is empty.")
        return
    ...
    save_path = (
        Path(save_path) if save_path is not None else Path(frame_paths[0]).parent
    )

    # video specs for writing video
    eg_frame = cv2.imread(Path(frame_paths[0]).as_posix())
    ...
    output_v_path = _get_output_video_path(
        save_path=save_path, video_name=video_name, file_ext=file_ext
    )
    ...
    for i, frame_path in enumerate(tqdm(frame_paths, desc="Plotting frames")):
        frame_id = i
        img = cv2.imread(Path(frame_path).as_posix())
        dets = mot_anno[mot_anno[:, 0] == frame_id]
        plotted_frame = plot_mot_bbox_on_image(img=img, mot_dets=dets)
        vw.write(plotted_frame)
...
def _get_output_video_path(
    save_path: Union[str, Path], video_name: str, file_ext: str
) -> Path:
    ...
    if output_v_path.exists():
        warning_msg = f"output file already exists: {output_v_path}"
        for n in range(1, 100):
            output_v_path = output_dir_path / f"{video_name}_plotted_{n}{file_ext}"
            if not output_v_path.exists():
                break
        warning_msg += f", changing to {output_v_path}"
    return output_v_path
